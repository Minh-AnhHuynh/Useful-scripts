...
def get_download_directory():
	...
	if os.path.exists(config_file_path):
		...
		download_dir = input(
			"Enter the default download directory. Leave blank for the current directory: "
		)
		# Default to the current directory if input is blank
		if not download_dir:
...
# Run the scidownl command based on the clipboard content
...
if content_type == "doi":
	...
	subprocess.run([
		"scidownl", "download", "--link", clipboard_content, "--out",
		download_dir
	])
else:
